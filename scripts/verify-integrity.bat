...
if not exist "%SHA_FILE%" (
    ...
    curl -sLO "https://github.com/tilas01/unix-dynamic-guides/releases/latest/download/%SHA_FILE%"
)
if not exist "%ASC_FILE%" (
    echo [i] Missing .asc file. Attempting to download...
    curl -sLO "https://github.com/tilas01/unix-dynamic-guides/releases/latest/download/%ASC_FILE%"
)
...
if exist "%ASC_FILE%" (
    ...
    if not exist "%PUB_KEY%" (
        ...
        curl -sL "https://raw.githubusercontent.com/tilas01/unix-dynamic-guides/main/tilas01-public-key.asc" -o "%PUB_KEY%"
    )
