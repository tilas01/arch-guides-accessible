...
if [ ! -f "${BINARY}.sha256" ]; then
    ...
    curl -sLO "https://github.com/tilas01/unix-dynamic-guides/releases/latest/download/${BINARY}.sha256"
fi

if [ ! -f "${BINARY}.asc" ]; then
    echo "⚠️ Warning: ${BINARY}.asc not found. Attempting to download..."
    curl -sLO "https://github.com/tilas01/unix-dynamic-guides/releases/latest/download/${BINARY}.asc"
fi
...
if ! command -v gpg &> /dev/null; then
    ...
    if [ ! -f "tilas01.asc" ]; then
        ...
        curl -sLO "https://raw.githubusercontent.com/tilas01/unix-dynamic-guides/main/tilas01.asc"
    fi
