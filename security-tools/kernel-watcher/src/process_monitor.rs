            ...
            for (pid, process) in sys.processes() {
                ...
                for &suspicious in SUSPICIOUS_PROCESSES {
                    if name.contains(suspicious) {
                        ...
                        println!(
                            "[ALERT] Unauthorized Screen Recording tool detected: {} (PID: {})",
                            name, native_pid
                        );
                        ...
                        if signal::kill(Pid::from_raw(native_pid), Signal::SIGSTOP).is_ok() {
                            ...
                            if authorize_process(&name) {
                                println!(
                                    "[ACTION] Authorization granted. Resuming process (SIGCONT)."
                                );
                                let _ = signal::kill(Pid::from_raw(native_pid), Signal::SIGCONT);
                            } else {
                                println!(
                                    "[ACTION] Authorization failed. Terminating process (SIGKILL)."
                                );
...
fn authorize_process(process_name: &str) -> bool {
    ...
    println!(
        " The application '{}' is attempting to record or share the screen.",
        process_name
    );
    println!("===========================================================");
